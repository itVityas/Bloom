...
def process_product(invoice_item: ClearanceInvoiceItems, order_list: list, is_gifted: bool) -> Tuple:
    ...
    if model.production_code.code == 400:
        if order_list:
            ...
            declaration_numbers = Declaration.objects.filter(
                is_use=True).values_list('declaration_number')
        if is_gifted:
            declaration_numbers = declaration_numbers.filter(gifted=True, is_use=True).values_list('declaration_number')
            products = products.filter(consignment__declaration_number__in=declaration_numbers)
        else:
            declaration_numbers = declaration_numbers.filter(
    ...
    if request_quantity > products_quantity:
        ...
        raise ProductsNotEnoughException(
            have_count=products_quantity,
            req_count=request_quantity,
            model_name=invoice_item.model_name_id.name)
