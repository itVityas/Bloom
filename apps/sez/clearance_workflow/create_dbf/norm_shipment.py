...
def generate_norm_dbf_shipment(clearance_invoice_id: int, output_path: str, encoding: str = 'cp866') -> None:
    """
    Generate a NORM-format DBF file for a given ClearanceInvoice for reverse shipment of delivery.
    ...
    cleared_items = ClearedItem.objects.filter(
        clearance_invoice_items__clearance_invoice_id=clearance_invoice_id
    ).order_by('id')
    ...
    with transaction.atomic():
        for item_index, item in enumerate(cleared_items, start=1):
