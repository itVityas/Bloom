...
def generate_rashod_tovar_decl_dbf_shipment(
    clearance_invoice_id: int,
    output_path: str,
    encoding: str = 'cp866'
) -> None:
    """
    Generate a RASHOD_tovar-format DBF file for a given ClearanceInvoice  for reverse shipment of delivery.
    ...
    clead_items = ClearedItem.objects.filter(clearance_invoice_items__clearance_invoice=invoice).order_by('id')
    line = 1
    for cleared_item in clead_items:
        ...
        for name, ftype, *_ in TOVAR_RASHOD_FIELDS:
            if name == 'G312':
                ...
                value = cleared_item.declared_item_id.code
            elif name == 'G38':
                value = 1
            elif name == 'G45':
                value = 1
