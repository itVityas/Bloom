...
def process_all_dbf_files(zip_file_path):
    """
    ...
    If any of these required files is missing, an Exception is raised with a message
    listing the missing file names.

    For each file present in the archive, the corresponding processing function is called
    with the path to the extracted temporary file. All temporary files (including the original zip file)
